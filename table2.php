<?php
 ...
 $database = $factory->createDatabase();
        ...
        $postRef = $database->getReference('table2')->remove();

        for($i=0; $i<7; $i++){

        $sql1 = "SELECT * FROM lect2  where day = '$day_arr[$i]'";
        $sql2 = "SELECT * FROM sec2  where Dayy = '$day_arr[$i]'";
 ...
 if ($res) {
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $postRef = $database->getReference('table2')->push( $str );
    }
}
if ($res2) {
